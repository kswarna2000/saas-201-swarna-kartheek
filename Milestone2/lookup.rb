...
def parse_dns(dns_raw)
  ...
  dns_raw.
    ...
    select { |row| row.length == 3 }.each { |record_array| #checks for the invalid records
    record_type = record_array[0]
    domain = record_array[1]
    value = record_array[2]
    if a[record_type] == nil
      a[record_type] = {} #creating a hash
    end
    a[record_type][domain] = value #storing records as hash of hashes
  }
  return a
end
